...
public class DatabasePostgres : IDatabase
{
    ...
    {
        var result = new List<(string word, int id, int frequency)>();

        var selectCmd = _connection.CreateCommand();
        selectCmd.CommandText =
            "SELECT w.name, w.id, COUNT(o.wordId) as freq " +
            "FROM occ o JOIN word w ON o.wordId = w.id " +
