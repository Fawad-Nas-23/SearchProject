...
public class DatabasePostgres : IDatabase
{
    ...
    private readonly ILogger<DatabasePostgres> _logger;

    public DatabasePostgres(IConfiguration configuration, ILogger<DatabasePostgres> logger)
    {
        _logger = logger;

        var dbPath = configuration["POSTGRES_DATABASE"] ?? Paths.POSTGRES_DATABASE;
        _logger.LogInformation("Connecting to PostgreSQL database at {DbPath}", dbPath);

        try
        {
            _connection = new NpgsqlConnection(dbPath);
            _connection.Open();
            _logger.LogInformation("PostgreSQL connection opened successfully");
        }
        catch (Exception ex)
        {
            _logger.LogCritical(ex, "Failed to open PostgreSQL database at {DbPath}", dbPath);
            throw;
        }
    }
